...
const ItemDisplay = ({items, addToCart ,item, setQuantity, cartItems}) => {  
    ...
    return (
        <div id='itemDisplay' className=' m-10 flex flex-row justify-center items-center gap-[1vmin] flex-wrap w-[75%] border-2 border-orange-300 p-[5vmin] rounded-xl bg-transparent shadow-xl'>
        {   
            item?
                <Card id='productDetails'  className=' w-[40vmin] shadow-xl bg-orange-300 flex flex-col justify-between rounded-xl border-5 border-orange-800' >
                ...
                <Card.Body classsName=''>
                        ...
                        <Button id='addToCartBtn' onClick={()=> handleAddToCart(item.id)} variant="success">Add to Cart</Button>
                    </Card.Body>
                </Card>
            :
            items.length > 0 ?
                ...
                <Card id='product'  className=' border-2 w-[50vmin] shadow-xl  bg-transparent flex flex-col justify-between rounded-xl ' > 
                    ...
                    <Card.Body className='flex flex-col justify-between '>
                        ...
                        <div className='flex flex-row justify-between'>
                            <Button id='viewDetailsBtn' onClick={()=> navigate(`details/${item.id}/`)} variant="primary">View Details</Button>
                            {!item.sold? <Button id='addToCartBtn' onClick={()=> handleAddToCart(item.id)} variant="success">Add to Cart</Button> : <Button id='addToCartBtn' onClick={()=> handleAddToCart(item.id)} disabled='true' variant="success">Add to Cart</Button>}
                        </div>
                    </Card.Body>
                </Card>
                )
